...
exp.get_image_hash = function*(rid, userId, type) {
  let cache_details = yield cache.get_details(userId);
...
exp.get_skin = function*(rid, userId) {
  ...
  let slim = data.slim;
  if (skin_hash) {
    let skinpath = path.join(config.directories.skins, skin_hash + ".png");
    try {
      yield fs.accessAsync(skinpath);
      logging.debug(rid, "skin already exists, not downloading");
      let img = yield skins.open_skin(rid, skinpath);
      return {
        hash: skin_hash,
        status,
        image: img,
      ...
      let data2 = yield promisify(networking.save_texture)(rid, skin_hash, skinpath);
      return {
        hash: skin_hash,
        status,
        image: data2.img,
...
exp.get_render = function*(rid, userId, scale, overlay, body) {
  ...
  let renderpath = path.join(config.directories.renders, [skin_hash, scale, get_type(overlay, body), slim ? "s" : "t"].join("-") + ".png");
  try {
