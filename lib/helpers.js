...
exp.get_avatar = function(rid, userId, overlay, size, callback) {
  return new Promise(function(resolve, reject) {
    exp.get_image_hash(rid, userId, "skin", function(err, data) {
      if (err) {
        return reject(err);
      }
      ...
      if (skin_hash) {
        ...
        fs.access(helmpath, function(fs_err) {
          ...
          skins.resize_img(filepath, size)
            .then(image => {
              return resolve({
                image,
                hash: skin_hash,
        ...
        // fs.accessAsync(helmpath)
        //     .then(() => skins.resize_img(helmpath, size))
        //     .then(image => {
        //         resolve({
        //             image,
        //             hash: skin_hash
...
exp.get_skin = function(rid, userId) {
  return promisify(exp.get_image_hash)(rid, userId, "skin")
    .then(function(data) {
      let status = data.status;
      let skin_hash = data.hash;
      let slim = data.slim;
      if (skin_hash) {
        ...
        return fs.accessAsync(skinpath)
          .then(function() {
            logging.debug(rid, "skin already exists, not downloading");
            return promisify(skins.open_skin)(rid, skinpath)
              .then(function(img) {
                return {
                  hash: skin_hash,
                  status,
...
exp.get_cape = function(rid, userId, callback) {
  exp.get_image_hash(rid, userId, "cape", function(err, data) {
    let status = data.status;
    ...
    fs.access(capepath, function(fs_err) {
      if (!fs_err) {
        ...
        networking.save_texture(rid, cape_hash, capepath, function(net_err, data) {
