...
let handle_default = (scale, overlay, body, img_status, userId, size, def) => {
  ...
  try {
    let data = yield promisify(helpers.get_render)(this.id, userId, scale, overlay, body)
      ...
      if (data.image) {
        ...
        handle_default(scale, overlay, body, status, userId, scale, def);
      }
  } catch(e) {
    handle_default(scale, overlay, body, -1, userId, scale, def);
  }
};
